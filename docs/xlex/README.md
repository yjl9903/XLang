...
XLex 是一个词法分析器工具，它在设计时一定程度上借鉴了 Flex。你可以使用它构造自己的词法分析规则，并用它来分析输入的代码文本串，生成 Token 流。XLex 借助 JavaScript 脚本允许你对分析过程做更多的定制。
...
XLex是一个基于有限状态自动机的词法分析器，它使用词法规则的配置对象（命令行工具中使用动态加载 JavaScript 脚本的形式传入）进行初始化。

XLex 使用一个递归下降分析器来解析配置信息中的 [正则表达式](/xlex/reg)，构建出一个非确定性有限状态自动机，使用子集构造法将其确定化为确定性有限状态自动机，使用 Hopcroft 算法将 DFA 最小化。
